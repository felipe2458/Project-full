$(function(){
    ...
    socket.on('verificar_player', (data)=>{
        ...
        data.forEach(player =>{
            if($(`#user_${player.name}`).length === 0){
                const user_icon_data = users_icons.find(icon => icon.name === player.name);
                const iconUrl = user_icon_data.icon ? user_icon_data.icon : '/public/user_icons/iconDefault/icon-padrão.webp';

                container_users.append(`
                    <div class="user-wraper" id="user_${player.name}">
